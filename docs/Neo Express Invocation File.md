...
- `hash` is the hex-encoded script hash of the contract to invoke. A contract's script
  hash can be calculated from its `.nef` file with the `neoxp contract hash` command, or
  listed for already-deployed contracts with `neoxp contract list`.
