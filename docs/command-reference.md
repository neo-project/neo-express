...
[Options]:
  ...
  -f|--force                          Deploy despite a name conflict, or update the
                                      existing contract when the hash already matches
  -g|--gas <GAS>                      Additional GAS applied to the deployment
                                      Default: 1
  -j|--json                           Output as JSON
...
`--force` on the **same** hash calls the contract `update` method (which must invoke
`ContractManagement.Update` internally). There is no `update` method → error (reset or restore,
then deploy).
