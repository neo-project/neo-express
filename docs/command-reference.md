...
### neo-express contract import

The `contract import` command imports a smart contract AVM file for use in a NEO-Express
blockchain. The command takes a path to an .AVM file generated by a NEO contract
compiler like [neon compiler for .NET](https://github.com/neo-project/neo-devpack-dotnet).

Importing a contract also reads the .abi.json file generated by the neon compiler.
This file is required as it provides parameter type information that is used when
invoking the contract.

If the user wants to overwrite a contract that has already been imported, they must
specify the `--force|-f` option. Note, this command does not modify the blockchain
data. So any contracts that are deployed to the blockchain are not changed.

Once a contract has been imported, it is reference to by the filename of the avm
file without path or extension. For example, '/someDirectory/myContract.avm' would
be referred to as 'myContract' for get, deploy and invoke.
...
The `contract deploy` command deploys an imported contract to a running NEO-Express
